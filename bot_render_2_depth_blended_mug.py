...
def main(args):
    ...
    for split in splits:
        all_pair_filenames_json = os.path.join(data_root_path, f"bot_render_2_{split}_mug_pair_filenames.json")
        with open(all_pair_filenames_json, "r") as f:
            data_dict = json.load(f)

        cur_split_path = os.path.join(data_root_path, f"{split}_mug_crop")
