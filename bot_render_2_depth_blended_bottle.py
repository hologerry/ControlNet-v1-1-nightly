...
def main(args):
    ...
    for split in splits:
        all_pair_filenames_json = os.path.join(data_root_path, f"bot_render_2_{split}_bottle_pair_filenames.json")
        with open(all_pair_filenames_json, "r") as f:
            data_dict = json.load(f)

        cur_split_path = os.path.join(data_root_path, f"{split}_bottle_crop")
        cur_split_output_path = os.path.join(
            data_root_path, f"{split}_bc_bottle_dial{args.dilation_radius}_seed{args.seed}"
        )
        cur_split_pairs = data_dict["pairs"]
        ...
        for pair in tqdm(cur_job_pairs, desc=desc_str):
            ...
            color_filename = pair["color"]
            mask_filename = pair["mask"]

            depth_filename = pair["depth"]
